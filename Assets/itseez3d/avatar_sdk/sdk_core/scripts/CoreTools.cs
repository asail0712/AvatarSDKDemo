...
#region Version and Platform
		...
		{
			bool isDeveloperMode = Directory.Exists("Assets/itseez3d_dev");
			bool isCloudExist = Directory.Exists("Assets/itseez3d/avatar_sdk/sdk_cloud");
			bool isLocalComputeExist = Directory.Exists("Assets/itseez3d/avatar_sdk/sdk_local_compute");
			bool isCloudUmaExist = Directory.Exists("Assets/itseez3d/avatar_sdk_uma/cloud");
			bool isLocalComputeUmaExist = Directory.Exists("Assets/itseez3d/avatar_sdk_uma/local_compute");

			List<Flavour> flavours = new List<Flavour>();

			if (isCloudExist)
				flavours.Add(Flavour.CLOUD);
			if (isLocalComputeExist)
				flavours.Add(Flavour.LOCAL_COMPUTE);
			if (isCloudUmaExist)
			{
				flavours.Add(Flavour.CLOUD_UMA);
				if (!isDeveloperMode)
					flavours.Remove(Flavour.CLOUD);
			}
			if (isLocalComputeUmaExist)
			{
				flavours.Add(Flavour.LOCAL_COMPUTE_UMA);
				if (!isDeveloperMode)
					flavours.Remove(Flavour.LOCAL_COMPUTE);
			}
...
#region Export functionality
		...
		public static List<string> FindExistedPBRTextures(string avatarId)
		{
			List<string> pbrTextures = new List<string>();
			var storage = AvatarSdkMgr.Storage();
			List<AvatarFile> pbrTexturesFiles = new List<AvatarFile>() { AvatarFile.METALLIC_MAP_TEXTURE, AvatarFile.ROUGHNESS_MAP_TEXTURE, AvatarFile.NORMAL_MAP_TEXTURE };
			foreach (AvatarFile avatarFile in pbrTexturesFiles)
			{
				string texturePath = storage.GetAvatarFilename(avatarId, avatarFile);
				if (File.Exists(texturePath))
					pbrTextures.Add(texturePath);
			}
			return pbrTextures;
		}
